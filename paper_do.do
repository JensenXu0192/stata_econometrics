...
*Robustness check
eststo clear
estpost tabstat lnWAGES FEMALE FEM_AGE FEM_AGE_2 FEM_MAR FEM_INFANT FEM_PRESCHOOLER FEM_SCHOOLAGE FEM_YOUTH FEM_MIN INFANT PRESCHOOLER SCHOOLAGE YOUTH MARRIED AGE AGE_2 NAIC HDGREE PR WEEKWRK MINORITY, listwise statistics(mean sd count min max) columns(statistics)
esttab using "sumst_rob.rtf", replace cells("mean(fmt(a3)) sd count min max") title(Table 2) label noobs

reg lnWAGES FEMALE FEM_AGE FEM_AGE_2 FEM_MAR FEM_INFANT FEM_PRESCHOOLER FEM_SCHOOLAGE FEM_YOUTH FEM_MIN INFANT PRESCHOOLER SCHOOLAGE YOUTH MARRIED AGE AGE_2 i.NAIC i.HDGREE i.PR WEEKWRK MINORITY
estimates store model_rob

*Produce a aggregated table for regression outputs
esttab model_int model_rob using "regout.rtf", replace label compress order(FEMALE FEM_POTENEXP FEM_POTENEXP_2 FEM_AGE FEM_AGE_2 FEM_MAR FEM_INFANT FEM_PRESCHOOLER FEM_SCHOOLAGE FEM_YOUTH FEM_MIN FEM_MAR FEM_INFANT FEM_PRESCHOOLER FEM_SCHOOLAGE FEM_YOUTH FEM_MIN YOUTH MARRIED POTENEXP POTENEXP_2 AGE AGE_2 MINORITY) b(3) title(Table 3) varwidth(40)
